...
class PathPlatformController(ROSNode):
    ...
    def __compute_turning_point(self, angle_delta:float) -> Optional[float]:
        ...
        if angle_delta > 0.01:
            turn_radius = -2.0 * (1.0-max(2*angle_delta/0.3, 1)) - (PlatformStatics.ROBOT_WIDTH/2 + 0.05)
        elif angle_delta < 0.01:
            turn_radius = 2.0 * (1.0+min(2*angle_delta/0.3, -1)) + (PlatformStatics.ROBOT_WIDTH/2 + 0.05)
        else:
            turn_radius = 0
