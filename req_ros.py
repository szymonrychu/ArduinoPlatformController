...
class FireForgetPublisher():
   def __init__(self, topic:str, type:Type, data:genpy.Message):
      ...
      rospy.Timer(rospy.Duration.from_sec(0.1), callback=self.__publish, oneshot=True)
      rospy.spin()
      self.__publisher = rospy.Publisher(topic, data_class=type)
      self.__data = data

   def __publish(self, *_args, **_kwargs):
      self.__publisher.publish(self.__data)
      time.sleep(1)
