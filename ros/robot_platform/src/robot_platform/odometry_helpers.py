...
def create_requests(duration:float, platform_status:PlatformStatus, velocity:float = PlatformStatics.MOVE_VELOCITY, turn_duration:Optional[float] = None, turning_point:Optional[Point]=None, tilt:float=0.0, pan:float=0.0) -> List[Request]:
    ...
    if current_turning_point != None:
        ...
        request.motor1 = Motor(velocity = 1.5 * round(velocity_coefficients[0] * velocity, 3))
        request.motor2 = Motor(velocity = 1.5 * round(velocity_coefficients[1] * velocity, 3))
        request.motor3 = Motor(velocity = 1.5 * round(velocity_coefficients[2] * velocity, 3))
        request.motor4 = Motor(velocity = 1.5 * round(velocity_coefficients[3] * velocity, 3))
        return [ request ]
    else:
        ...
        turn_request.servo2 = Servo.from_ROS_ServoStatus_and_delta_angle(platform_status.servo2, motor_servo_angle_deltas[1])
        turn_request.servo3 = Servo.from_ROS_ServoStatus_and_delta_angle(platform_status.servo3, motor_servo_angle_deltas[2])
        turn_request.servo4 = Servo.from_ROS_ServoStatus_and_delta_angle(platform_status.servo4, motor_servo_angle_deltas[3])
