...
def create_requests(duration:float, platform_status:PlatformStatus, velocity:float = PlatformStatics.MOVE_VELOCITY, turn_duration:Optional[float] = None, turning_point:Optional[Point]=None, tilt:float=0.0, pan:float=0.0) -> List[Request]:
    ...
    if current_turning_point != None:
        ...
        request.motor1 = Motor(velocity = -2 * round(velocity_coefficients[0] * velocity, 3))
        request.motor2 = Motor(velocity = -2 * round(velocity_coefficients[1] * velocity, 3))
        request.motor3 = Motor(velocity = -2 * round(velocity_coefficients[2] * velocity, 3))
        request.motor4 = Motor(velocity = -2 * round(velocity_coefficients[3] * velocity, 3))
        return [ request ]
    else:
